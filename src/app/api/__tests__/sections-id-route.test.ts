...
/**
 * Unit tests for /api/sections/[id]/route.ts
 * Covers PATCH and DELETE logic, including error handling and audit logging.
 */
import { it, expect, vi, beforeEach } from 'vitest';
import * as route from '../sections/[id]/route';
...
function makeReq(method: string, jsonData?: any): any {
  const nextUrl = new URL('http://localhost:3000/api/sections/1');
  return {
    method,
    cookies: {},
...
it('PATCH updates section and logs audit', async () => {
  const req = makeReq('PATCH', { code: 'NEWCODE' });
...
it('DELETE removes section and logs audit', async () => {
...
// Error handling test removed: Vitest requires only top-level static mocks.
