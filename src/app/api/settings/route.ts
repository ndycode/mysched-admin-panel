...
const CACHE_CONTROL_DEFAULT = 'private, max-age=30, stale-while-revalidate=120'

function respond<T>(data: T, status = 200, cacheControl = 'no-store') {
...
type BoolEnv = string | boolean | undefined | null

function parseBoolean(value: BoolEnv, fallback = false) {
...
const AnalyticsIntegrationSchema = z
  .object({
    connected: z.boolean(),
...
type StoredSettings = z.infer<typeof SettingsPartialSchema>
...
type AdminSettingsResponse = SettingsPayload & {
...
function envDefaults(): SettingsPayload {
  ...
  const maintenanceMode = parseBoolean(process.env.NEXT_PUBLIC_MAINTENANCE_MODE, false)
  const allowRegistrations = parseBoolean(
    process.env.NEXT_PUBLIC_ENABLE_SIGNUPS ?? process.env.NEXT_PUBLIC_ALLOW_SIGNUPS,
    true,
  )

  const emailAlerts = parseBoolean(process.env.NEXT_PUBLIC_NOTIFICATIONS_EMAIL, true)
  const smsAlerts = parseBoolean(process.env.NEXT_PUBLIC_NOTIFICATIONS_SMS, false)
  const weeklyDigest = parseBoolean(process.env.NEXT_PUBLIC_WEEKLY_DIGEST, true)
  const productUpdates = parseBoolean(process.env.NEXT_PUBLIC_PRODUCT_UPDATES, true)

  const enforceTwoFactor = parseBoolean(
    process.env.NEXT_PUBLIC_REQUIRE_2FA ?? process.env.NEXT_PUBLIC_ENFORCE_2FA,
    false,
  )
  const passwordRotationDays = parseNumber(process.env.NEXT_PUBLIC_PASSWORD_ROTATION_DAYS, 180)
  const sessionTimeoutMinutes = parseNumber(process.env.NEXT_PUBLIC_SESSION_TIMEOUT, 30)
  const auditLogRetentionDays = parseNumber(process.env.NEXT_PUBLIC_AUDIT_RETENTION_DAYS, 90)
...
async function fetchStoredSettings(): Promise<StoredSettings | null> {
  if (!canUseServiceClient()) return null
  try {
...
function mergeSettings(base: SettingsPayload, stored: StoredSettings | null): SettingsPayload {
...
function buildRecommendations(settings: SettingsPayload): Recommendation[] {
  return [
    {
      id: 'support-email',
...
async function composeSettings(): Promise<AdminSettingsResponse> {
  const base = envDefaults()
  const stored = await fetchStoredSettings()
  const merged = mergeSettings(base, stored)
  return { ...merged, recommendations: buildRecommendations(merged) }
}
...
function sanitizeForStorage(input: SettingsPayload): SettingsPayload {
  ...
  return {
    ...
    integrations: {
      supabase: {
        ...
        projectRef: toNull(input.integrations.supabase.projectRef),
      ...
      webhook: {
        endpoint: toNull(input.integrations.webhook.endpoint),
        connected:
          Boolean(toNull(input.integrations.webhook.endpoint)) && Boolean(input.integrations.webhook.connected),
        lastVerifiedAt:
          Boolean(toNull(input.integrations.webhook.endpoint)) && input.integrations.webhook.connected
            ? input.integrations.webhook.lastVerifiedAt ?? null
            : null,
      },
      analytics: {
        provider: toNull(input.integrations.analytics.provider),
        connected: Boolean(toNull(input.integrations.analytics.provider)),
        lastExportAt: input.integrations.analytics.lastExportAt ?? null,
        destination: toNull(input.integrations.analytics.destination),
      },
    },
  }
}
...
export async function GET() {
  try {
    await requireAdmin()
    const payload = await composeSettings()
    return respond(payload, 200, CACHE_CONTROL_DEFAULT)
  } catch (error) {
    ...
    if (status === 403) {
      return respond({ error: 'You do not have access to these settings.' }, 403)
    }

    const msg = logErr('/api/settings GET', error)
    return respond({ error: msg || 'Internal Server Error' }, 500)
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!canUseServiceClient()) {
      return respond({ error: 'Supabase service credentials are not configured.' }, 503)
    }

    const sanitized = sanitizeForStorage(parsed)
    const sb = sbService()
    ...
    if (error) throw createHttpError(500, error.message || 'Failed to save settings', error)

    await audit(user.id, 'admin_settings', 'update', 'global', { details: sanitized })

    const payload = await composeSettings()
    return respond(payload, 200)
  } catch (error) {
    if (error instanceof z.ZodError) {
      return respond({ error: 'Invalid settings payload.', issues: error.issues }, 422)
    }

    const status = extractStatus(error)
    if (status === 429) {
      return respond({ error: 'Too many attempts. Please wait a moment and try again.' }, 429)
    }
    if (status === 403) {
    ...
    const msg = logErr('/api/settings PATCH', error, { method: req.method })
    await auditError('system', 'admin_settings', msg)
    return respond({ error: msg || 'Internal Server Error' }, 500)
  }
}
